...
describe Liftoff::TemplateFinder do
  describe '#template_path' do
    ...
    it 'returns the user file if it exists' do
      File.stub(:exists?).with(user_file('user')) { true }
      File.stub(:exists?).with(local_file('user')) { false }
    ...
    it 'returns the default template if there is no local or user template' do
      File.stub(:exists?).with(user_file('default')) { false }
      File.stub(:exists?).with(local_file('default')) { false }
